"""Ping students who've submitted an invalid username.
...
class PingInvalidUsername(FlowPropagateStep[StudentRecord]):
    ...
    def should_ping(self: Self, record: StudentRecord) -> bool:
        ...
        if record.last_valid_ping:
            # Always round to the hour the ping happened in, to avoid random
            # drift over time
            last_ping = record.last_valid_ping.replace(
    ...
    def propagate_records(
        ...
        """Email students with an invalid GitHub username in their record.
        ...
        for record, lock in records:
            with lock:
                if (
                    ...
                    and (record.github_valid is False)
                    ...
                    if debug:
                        ...
                        email_body = (
                            ...
                            .replace("<username>", record.github_username)
                        )
                        ...
                        record.last_valid_ping = datetime.now()
                        logger(
